...
const History = () => {
    ...
    useEffect(() => {
        // Fetch all history and politics books
        axios.get('http://localhost:4000/item')
            .then((response) => {
                const allBooks = response.data;
                const historyBooksFiltered = allBooks.filter(book => book.genre.toLowerCase() === 'history');
                setHistoryBooks(historyBooksFiltered);
            })
            .catch((error) => {
    ...
    const addToWishlist = async (itemId) => {
        try {
            const selectedItem = historyBooks.find((book) => book._id === itemId);
    ...
    return (
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="main-content">
                <h1 className="text-center" style={{ fontSize: '36px', marginTop: '20px' }}>History & Politics Books</h1>
