...
const Children = () => {
  ...
  useEffect(() => {
    // Fetch all books and filter by genre 'young adult'
    axios
      .get('http://localhost:4000/item')
      .then((response) => {
        const allBooks = response.data;
        const childrenBooks = allBooks.filter(book => book.genre.toLowerCase() === 'young adult');
        setBooks(childrenBooks);
      })
      .catch((error) => {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="main-content">
        <h1 className="text-center" style={{ fontSize: '36px', marginTop: '20px' }}>Young Adult Books</h1>
